package com.shahtott.alexon_task.util.network
...
fun getHttpErrorMessage(code: Int): String {
    ...
    return getRealErrorMessage(httpErrorType)
}

fun getRealErrorMessage(httpErrorType: HttpErrorType): String {
    return when (httpErrorType) {
        HttpErrorType.BadGateway -> "Server is down meanwhile"
        is HttpErrorType.BadRequest -> "Some of your request data might have incorrect syntax"
        is HttpErrorType.DataInvalid -> "Invalid data , Please insert valid data"
        HttpErrorType.Forbidden -> "You don't have permission to request URL"
        HttpErrorType.InternalServerError -> "API server is crashed"
        HttpErrorType.NotAuthorized -> "You haven't authenticated the API"
        HttpErrorType.NotFound -> "Your request doesn't exist"
        HttpErrorType.Unknown -> "Unknown Error"
    }
}
